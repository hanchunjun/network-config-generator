...
class ConfigGenerator:
    ...
    _VENDOR_CONFIG_MAP: Dict[str, Dict[str, Callable]] = {
        'access_switch': {
            'huawei': '_generate_huawei_access_switch_config',
            'ruijie': '_generate_ruijie_access_switch_config',
            'h3c': '_generate_h3c_access_switch_config',
            'cisco': '_generate_cisco_access_switch_config',
        },
        'core_switch': {
            'huawei': '_generate_huawei_core_switch_config',
            'ruijie': '_generate_ruijie_core_switch_config',
            'h3c': '_generate_h3c_core_switch_config',
            'cisco': '_generate_cisco_core_switch_config',
        },
        'router': {
            'huawei': '_generate_huawei_router_config',
            'ruijie': '_generate_ruijie_router_config',
            'h3c': '_generate_h3c_router_config',
            'cisco': '_generate_cisco_router_config',
        },
        'ac': {
            'huawei': '_generate_huawei_ac_config',
            'ruijie': '_generate_ruijie_ac_config',
            'h3c': '_generate_h3c_ac_config',
            'cisco': '_generate_cisco_ac_config',
        },
    }

    @staticmethod
    def _sanitize_config_value(value: Any) -> str:
        """清理配置值，防止命令注入
    ...
    @staticmethod
    def _is_valid_vlan(vlan: str) -> bool:
        """验证VLAN ID格式
    ...
    @staticmethod
    def _get_config_function(device_type: str, vendor: str) -> Callable:
        """获取配置生成函数
        ...
        Raises:
            ValueError: 不支持的设备类型或厂商
        """
        try:
            method_name = ConfigGenerator._VENDOR_CONFIG_MAP[device_type][vendor]
            return getattr(ConfigGenerator, method_name)
        except KeyError:
    ...
    def _generate_huawei_access_switch_config(**kwargs):
        ...
        enable_password = kwargs.get('enable_password')
        if not enable_password:
            raise ValueError("必须提供enable特权密码")
        if len(enable_password) < 8:
            raise ValueError("密码长度必须至少8位")

        # 清理和验证所有用户输入
        hostname = ConfigGenerator._sanitize_config_value(kwargs.get('hostname', 'Switch'))
        mgmt_vlan = ConfigGenerator._sanitize_config_value(kwargs.get('mgmt_vlan', '100'))
        mgmt_ip = ConfigGenerator._sanitize_config_value(kwargs.get('mgmt_ip', '192.168.100.1'))
        mgmt_mask = ConfigGenerator._sanitize_config_value(kwargs.get('mgmt_mask', '255.255.255.0'))
        console_password = kwargs.get('console_password')
        ssh_user = ConfigGenerator._sanitize_config_value(kwargs.get('ssh_user', 'admin'))
        ssh_pwd = kwargs.get('ssh_pwd')
        user_vlan = ConfigGenerator._sanitize_config_value(kwargs.get('user_vlan', '10'))
        server_vlan = ConfigGenerator._sanitize_config_value(kwargs.get('server_vlan', '20'))
        up_port = ConfigGenerator._sanitize_config_value(kwargs.get('up_port', 'GigabitEthernet0/0/1'))
        down_port_range = ConfigGenerator._sanitize_config_value(kwargs.get('down_port_range', 'GigabitEthernet0/0/2 to GigabitEthernet0/0/24'))

        # 验证IP地址格式
        if not ConfigGenerator._is_valid_ip(mgmt_ip):
            raise ValueError(f"无效的管理IP地址: {mgmt_ip}")

        # 验证VLAN ID格式
        try:
            vlan_id = int(mgmt_vlan)
            if not (1 <= vlan_id <= 4094):
                raise ValueError(f"VLAN ID超出范围: {mgmt_vlan}")
        except ValueError:
            raise ValueError(f"无效的VLAN ID格式: {mgmt_vlan}")

        # 验证密码复杂度
        if console_password and len(console_password) < 8:
            raise ValueError("Console密码长度必须至少8位")
        if ssh_pwd and len(ssh_pwd) < 8:
            raise ValueError("SSH密码长度必须至少8位")
...
[HUAWEI] super password cipher {enable_password}
[HUAWEI] clock timezone BJ add 08:00:00
[HUAWEI] clock datetime {ConfigGenerator._sanitize_config_value(kwargs.get('system_time', '2024-01-01 00:00:00'))}

# 管理VLAN配置
[HUAWEI] vlan {mgmt_vlan}
[HUAWEI-vlan{mgmt_vlan}] name Management
[HUAWEI-vlan{mgmt_vlan}] quit
[HUAWEI] interface vlanif {mgmt_vlan}
[HUAWEI-Vlanif{mgmt_vlan}] ip address {mgmt_ip} {mgmt_mask}
[HUAWEI-Vlanif{mgmt_vlan}] quit
...
"""
        if console_password:
            config += f"[HUAWEI-ui-console0] set authentication password cipher {console_password}\n"
        config += """[HUAWEI-ui-console0] quit
...
"""
        if ssh_user and ssh_pwd:
            config += f"[HUAWEI-aaa] local-user {ssh_user} password cipher {ssh_pwd}\n"
            config += f"[HUAWEI-aaa] local-user {ssh_user} privilege level 15\n"
            config += f"[HUAWEI-aaa] local-user {ssh_user} service-type ssh\n"
        config += """[HUAWEI-aaa] quit
[HUAWEI] ssh server enable

# 业务VLAN创建
"""
        if user_vlan and server_vlan:
            config += f"[HUAWEI] vlan batch {user_vlan} {server_vlan}\n\n"

        config += f"""# 上联Trunk端口配置
[HUAWEI] interface {up_port}
[HUAWEI-{up_port}] port link-type trunk
[HUAWEI-{up_port}] port trunk allow-pass vlan all
[HUAWEI-{up_port}] quit

# 下联Access端口配置
[HUAWEI] interface range {down_port_range}
[HUAWEI-if-range] port link-type access
"""
        if user_vlan:
            config += f"[HUAWEI-if-range] port default vlan {user_vlan}\n"
        config += """[HUAWEI-if-range] quit
...
"""
        if user_vlan:
            config += f"[HUAWEI] vlan {user_vlan}\n"
            config += f"[HUAWEI-vlan{user_vlan}] dhcp snooping enable\n"
            config += f"[HUAWEI-vlan{user_vlan}] quit\n"
        config += f"""[HUAWEI] interface {up_port}
[HUAWEI-{up_port}] dhcp snooping trust
[HUAWEI-{up_port}] quit

# 运维监控配置
[HUAWEI] ntp-service unicast-server {ConfigGenerator._sanitize_config_value(kwargs.get('ntp_server', '123.123.123.123'))}
[HUAWEI] snmp-agent
[HUAWEI] snmp-agent community read {ConfigGenerator._sanitize_config_value(kwargs.get('snmp_ro_community', 'public'))}
[HUAWEI] snmp-agent target-host trap address udp-domain {ConfigGenerator._sanitize_config_value(kwargs.get('trap_server', '192.168.100.254'))} params securityname {ConfigGenerator._sanitize_config_value(kwargs.get('trap_community', 'public'))}
