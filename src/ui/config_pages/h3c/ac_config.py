...
class H3CACConfig(BaseConfigPage):
    def __init__(self, parent):
        super().__init__(parent, 'h3c', 'ac')
        self.form_fields = {}
        self.cards = []
        self.init_panels()
    ...
    def generate_config(self):
        ...
        if 'WLAN基本配置' in checked_cards:
            if data.get('wlan_id') and data.get('ssid_name'):
                config_commands.append(f"wlan service-template {data.get('wlan_id')} clear")
                config_commands.append(f"ssid {data.get('ssid_name')}")
                config_commands.append(f"bind WLAN-ESS 1")
                config_commands.append(f"service-template enable")
                config_commands.append(f"quit")
        
        if '安全策略配置' in checked_cards:
            if data.get('security_template'):
                config_commands.append(f"wlan sec-profile {data.get('security_template')}")
                if data.get('auth_method') == 'WPA2-PSK':
                    config_commands.append(f"wpa2-psk pass-phrase {data.get('psk_password')} aes")
                config_commands.append(f"quit")
        
        if 'VAP配置' in checked_cards:
            if data.get('vap_id') and data.get('vap_name'):
                config_commands.append(f"interface WLAN-ESS {data.get('vap_id')}")
                if data.get('bind_wlan'):
                    config_commands.append(f"port access vlan {data.get('bind_wlan')}")
                if data.get('security_template'):
                    config_commands.append(f"wlan sec-profile {data.get('security_template')}")
                config_commands.append(f"quit")
        ...
        if 'AP基础配置' in checked_cards:
            if data.get('ap_name') and data.get('ap_mac'):
                config_commands.append(f"wlan ap {data.get('ap_name')} model {data.get('ap_model', 'WA4320i-ACN')}")
                config_commands.append(f"serial-id {data.get('ap_mac')}")
                config_commands.append(f"quit")
        
        if 'AP分组配置' in checked_cards:
            if data.get('group_name'):
                config_commands.append(f"wlan ap-group {data.get('group_name')}")
        ...
        if 'CAPWAP隧道配置' in checked_cards:
            if data.get('ac_source_ip'):
                config_commands.append(f"capwap source ip {data.get('ac_source_ip')}")
        
        if '链路聚合配置' in checked_cards:
            if data.get('agg_id') and data.get('agg_ports'):
                config_commands.append(f"interface Bridge-Aggregation {data.get('agg_id')}")
                ...
                config_commands.append(f"port link-aggregation group {data.get('agg_id')}")
                config_commands.append(f"quit")
        ...
        if 'QoS配置' in checked_cards:
            if data.get('qos_template'):
                config_commands.append(f"wlan qos-profile {data.get('qos_template')}")
                if data.get('bandwidth_limit'):
                    config_commands.append(f"rate-limit client down {data.get('bandwidth_limit')}")
                config_commands.append(f"quit")
