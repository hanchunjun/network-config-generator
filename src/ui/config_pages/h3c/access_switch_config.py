...
class H3CAccessSwitchConfig(BaseConfigPage):
    ...
    def init_panels(self):
        ...
        self.ssh_user_input.setStyleSheet("""
            QLineEdit {
                border: 1px solid #CCCCCC;
                padding: 4px 8px;
                font-size: 10pt;
        ...
        self.ssh_user_pass_input.setStyleSheet("""
            QLineEdit {
                border: 1px solid #CCCCCC;
                padding: 4px 8px;
                font-size: 10pt;
        ...
        self.telnet_user_input.setStyleSheet("""
            QLineEdit {
                border: 1px solid #CCCCCC;
                padding: 4px 8px;
                font-size: 10pt;
        ...
        self.telnet_user_pass_input.setStyleSheet("""
            QLineEdit {
                border: 1px solid #CCCCCC;
                padding: 4px 8px;
                font-size: 10pt;
        ...
        port_dash_label.setStyleSheet('color: #4E5969; font-size: 10pt;')
    ...
    def generate_config(self):
        ...
        if 'SSH登录配置' in checked_cards:
            if data.get('ssh_user') and data.get('ssh_user_password'):
                ...
                config_commands.append(f"ssh server enable")
                config_commands.append(f"line vty 0 4")
                config_commands.append(f"password simple {data.get('ssh_password')}")
                config_commands.append(f"login")
                config_commands.append(f"protocol inbound ssh")
                config_commands.append(f"quit")
        
        if 'Telnet登录配置' in checked_cards:
            if data.get('telnet_user') and data.get('telnet_user_password'):
                config_commands.append(f"local-user {data.get('telnet_user')}")
                ...
                config_commands.append(f"line vty 0 4")
                config_commands.append(f"password simple {data.get('telnet_pwd')}")
                config_commands.append(f"login")
                config_commands.append(f"protocol inbound telnet")
                config_commands.append(f"quit")
            ...
            for row in range(self.access_table.rowCount()):
                ...
                if start_port and end_port and vlan:
                    if start_port == end_port:
                        ...
                        config_commands.append(f"interface range GigabitEthernet {interface}{start_port} to {interface}{end_port}")
        ...
        if '接口防环配置' in checked_cards:
            config_commands.append(f"loopback-detection global enable")
            config_commands.append(f"loopback-detection interval 120")
            if data.get('loop_interface') and data.get('loop_start_port') and data.get('loop_end_port'):
                ...
                config_commands.append(f"loopback-detection action shutdown")
                config_commands.append(f"quit")
                config_commands.append(f"quit")
        ...
        if '生成树配置 (STP/RSTP/MSTP)' in checked_cards:
            ...
            if data.get('bdpu_protection') and data.get('bdpu_protection') is True:
                config_commands.append(f"stp bpdu-protection")
        ...
        if 'ACL访问控制列表' in checked_cards:
            if hasattr(self, 'acl_table') and self.acl_table.rowCount() > 0:
                ...
                if data.get('acl_interface') and data.get('acl_start_port') and data.get('acl_end_port') and data.get('acl_direction'):
                    ...
                    config_commands.append(f"interface range {interface.replace(' ', '')}{start_port} to {interface.replace(' ', '')}{end_port}")
                    config_commands.append(f"packet-filter {acl_number} {direction}")
                    config_commands.append(f"quit")
        ...
        config.extend(["return", "save force"])
