...
class H3CCoreSwitchConfig(BaseConfigPage):
    def __init__(self, parent):
        super().__init__(parent, 'h3c', 'core_switch')
        self.form_fields = {}
        self.cards = []
    ...
    def init_panels(self):
        ...
        self.ssh_user_input.setStyleSheet(self._get_input_style())
        ...
        self.ssh_user_pass_input.setStyleSheet(self._get_input_style())
        ...
        self.telnet_user_input.setStyleSheet(self._get_input_style())
        ...
        self.telnet_user_pass_input.setStyleSheet(self._get_input_style())
        ...
        # 链路聚合配置卡片
        card8 = self.create_card('链路聚合 (Bridge-Aggregation)', '🔗', '配置端口聚合，支持静态和动态LACP模式')
        ...
        card8 = self.create_card('OSPF路由配置', '🌐', '配置OSPF路由进程、区域、网络宣告')
        self.add_form_item(card8, 'OSPF进程号', 'ospf_process')
        self.add_form_item(card8, '区域号', 'area_id')
        self.add_form_item(card8, '路由ID', 'router_id')
        self.add_form_item(card8, '网络地址', 'network_addr')
        self.add_form_item(card8, '子网掩码', 'network_mask')
        
        card9 = self.create_card('BGP路由配置', '🌐', '配置BGP路由进程、对等体、网络宣告')
        ...
        card10 = self.create_card('静态路由配置', '🌐', '配置静态路由')
        ...
        card11 = self.create_card('DHCP Snooping功能', '📶', '开启DHCP监听，防止伪DHCP服务器，配置信任端口')
        self.add_form_item(card11, '信任端口号', 'trust_port')
        
        card12 = self.create_card('接口防环配置', '🛡️', '开启RLDP防环功能，配置环路检测端口')
        self.add_form_item(card12, '防环端口范围', 'loop_port')
        
        card13 = self.create_card('生成树MSTP配置', '🌐', '配置MST多生成树实例，划分VLAN到不同实例')
        self.add_form_item(card13, 'MST实例1 VLAN', 'mst_vlan1')
        self.add_form_item(card13, 'MST实例2 VLAN', 'mst_vlan2')
        self.add_form_item(card13, '优先级', 'mst_prio')
        
        card14 = self.create_card('ACL访问控制列表', '📜', '配置访问控制列表，限制访问管理VLAN的来源')
        self.add_form_item(card14, '允许管理网段', 'allow_segment')
        self.add_form_item(card14, '允许主机', 'allow_host')
        self.add_form_item(card14, '拒绝网段1', 'deny_segment1')
        self.add_form_item(card14, '拒绝网段2', 'deny_segment2')
        ...
        self.tab_widget.addTab(tab4.widget, '🔒 安全与防环')
        
        # Tab5：运维与调试功能
        tab5 = self.create_tab_content()
        card15 = self.create_card('NTP时钟同步', '⏰', '配置NTP服务器，自动同步设备时钟')
        self.add_form_item(card15, 'NTP服务器地址', 'ntp_server')
        
        card16 = self.create_card('SNMPv2配置', '📈', '开启SNMP功能，配置团体名和Trap告警服务器')
        self.add_form_item(card16, 'SNMP只读团体名', 'snmp_community')
        self.add_form_item(card16, 'Trap服务器地址', 'trap_server')
        
        card17 = self.create_card('端口镜像配置', '📋', '配置端口镜像，将流量复制到监控端口')
        self.add_form_item(card17, '镜像源端口', 'mirror_src')
        self.add_combo_item(card17, '流量方向', 'mirror_dir', ['rx', 'tx', 'both'])
        self.add_form_item(card17, '镜像目的端口', 'mirror_dst')
        
        card18 = self.create_card('AAA本地认证配置', '🔐', '配置AAA本地认证，统一管理登录用户')
        self.add_form_item(card18, 'AAA用户名', 'aaa_user')
        self.add_form_item(card18, 'AAA密码', 'aaa_pwd', is_password=True)
        
        tab5.layout().addWidget(card15)
        tab5.layout().addWidget(card16)
        tab5.layout().addWidget(card17)
        tab5.layout().addWidget(card18)
        tab5.content_layout.addStretch()
        self.tab_widget.addTab(tab5.widget, '📊 运维与登录')
    ...
    def generate_config(self):
        ...
        if '客户端接入端口配置' in checked_cards:
            if data.get('access_port1') and data.get('access_vlan1'):
                config_commands.append(f"interface range GigabitEthernet {data.get('access_port1')}")
                config_commands.append(f"port access vlan {data.get('access_vlan1')}")
                config_commands.append(f"quit")
            if data.get('access_port2') and data.get('access_vlan2'):
                config_commands.append(f"interface range GigabitEthernet {data.get('access_port2')}")
                config_commands.append(f"port access vlan {data.get('access_vlan2')}")
                config_commands.append(f"quit")
        
        if '上联Trunk端口配置' in checked_cards:
            if data.get('up_port'):
                config_commands.append(f"interface GigabitEthernet {data.get('up_port')}")
                config_commands.append(f"port link-type trunk")
                config_commands.append(f"port trunk permit vlan all")
                config_commands.append(f"quit")
            if data.get('port'):
                config_commands.append(f"interface GigabitEthernet {data.get('port')}")
                ...
                config_commands.append(f"quit")
            if data.get('optic_port'):
                config_commands.append(f"interface GigabitEthernet {data.get('optic_port')}")
                if data.get('medium_type') == 'fiber':
                    config_commands.append(f"combo enable fiber")
                else:
                    config_commands.append(f"combo enable copper")
                config_commands.append(f"quit")
            ...
            for row in range(self.access_table.rowCount()):
                ...
                if interface and start_port and end_port and vlan:
                    ...
                    if start_port == end_port:
                        ...
                        config_commands.append(f"interface range {interface}{start_port} to {end_port}")
