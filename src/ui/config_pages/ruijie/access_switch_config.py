from PyQt5.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QLabel,
                               ...
                               QTableWidget, QTableWidgetItem, QHeaderView, QPushButton, QSizePolicy, QRadioButton, QButtonGroup)
...
class RuijieAccessSwitchConfig(BaseConfigPage):
    ...
    def init_panels(self):
        ...
        card_console.layout().addLayout(login_method_layout)
        ...
        username_input.setStyleSheet(self._get_input_alt_style())
        ...
        user_pwd_input.setStyleSheet(self._get_input_alt_style())
        ...
        # 系统时间配置卡片
        card_time = self.create_card('系统时间配置', '🕒', '配置系统时间和时区')
        self.add_form_item(card_time, '系统时间', 'system_time')
        self.add_form_item(card_time, '时区', 'timezone', is_label=True, default_value='北京时区 +8')
        
        card2 = self.create_card('VLAN 基础配置', '🌐', '批量创建 VLAN，支持连续范围如 2 to 10')
        # 添加输入提示
        ...
        # 登录方法选择布局（标题和单选按钮在同一行）
        ssh_login_method_layout = QHBoxLayout()
        ssh_login_method_layout.setSpacing(4)
        ...
        ssh_pwd_only_radio.setStyleSheet(self._get_radio_style())
        ...
        ssh_user_pwd_radio.setStyleSheet(self._get_radio_style())
        ssh_login_method_layout.addWidget(ssh_user_pwd_radio)
        ssh_login_method_group.addButton(ssh_user_pwd_radio, 1)
        
        ssh_login_method_layout.addStretch()
        card_ssh.layout().addLayout(ssh_login_method_layout)
        ...
        ssh_pwd_input.setStyleSheet(self._get_input_alt_style())
        ssh_method1_layout.addWidget(ssh_pwd_input)
        self.form_fields['ssh_password'] = ssh_pwd_input
        ...
        ssh_user_input = QLineEdit()
        ssh_user_input.setFixedWidth(140)
        ssh_user_input.setPlaceholderText('请输入用户名')
        ssh_user_input.setStyleSheet(self._get_input_alt_style())
        ssh_method2_layout.addWidget(ssh_user_input)
        ...
        ssh_user_pwd_input = QLineEdit()
        ssh_user_pwd_input.setEchoMode(QLineEdit.Password)
        ssh_user_pwd_input.setFixedWidth(140)
        ssh_user_pwd_input.setPlaceholderText('请输入密码')
        ssh_user_pwd_input.setStyleSheet(self._get_input_alt_style())
        ssh_method2_layout.addWidget(ssh_user_pwd_input)
        self.form_fields['ssh_user_password'] = ssh_user_pwd_input
        ...
        # 连接信号
        ssh_pwd_only_radio.toggled.connect(lambda checked: self.toggle_ssh_user_pass(checked))
        ssh_user_pwd_radio.toggled.connect(lambda checked: self.toggle_ssh_user_pass(not checked))
        ...
        # 登录方法选择布局（标题和单选按钮在同一行）
        telnet_login_method_layout = QHBoxLayout()
        telnet_login_method_layout.setSpacing(4)
        ...
        telnet_pwd_only_radio.setStyleSheet(self._get_radio_style())
        ...
        telnet_user_pwd_radio.setStyleSheet(self._get_radio_style())
        telnet_login_method_layout.addWidget(telnet_user_pwd_radio)
        telnet_login_method_group.addButton(telnet_user_pwd_radio, 1)
        
        telnet_login_method_layout.addStretch()
        card_telnet.layout().addLayout(telnet_login_method_layout)
        ...
        telnet_pwd_input.setStyleSheet(self._get_input_alt_style())
        telnet_method1_layout.addWidget(telnet_pwd_input)
        self.form_fields['telnet_password'] = telnet_pwd_input
        ...
        telnet_user_input = QLineEdit()
        telnet_user_input.setFixedWidth(140)
        telnet_user_input.setPlaceholderText('请输入用户名')
        telnet_user_input.setStyleSheet(self._get_input_alt_style())
        telnet_method2_layout.addWidget(telnet_user_input)
        ...
        telnet_user_pwd_input = QLineEdit()
        telnet_user_pwd_input.setEchoMode(QLineEdit.Password)
        telnet_user_pwd_input.setFixedWidth(140)
        telnet_user_pwd_input.setPlaceholderText('请输入密码')
        telnet_user_pwd_input.setStyleSheet(self._get_input_alt_style())
        telnet_method2_layout.addWidget(telnet_user_pwd_input)
        self.form_fields['telnet_user_password'] = telnet_user_pwd_input
        ...
        # 连接信号
        telnet_pwd_only_radio.toggled.connect(lambda checked: self.toggle_telnet_user_pass(checked))
        telnet_user_pwd_radio.toggled.connect(lambda checked: self.toggle_telnet_user_pass(not checked))
        
        # 添加卡片到tab1
        tab1.layout().addWidget(card1)
        tab1.layout().addWidget(card2)
        tab1.layout().addWidget(card3)
        tab1.layout().addWidget(card_time)  # 系统时间配置卡片移到最后
        ...
        # 至
        apply_layout.addWidget(QLabel('至'))
        ...
        acl_config_layout.addLayout(apply_layout)
        
        card11.layout().addLayout(acl_config_layout)
        
        add_rule_button.clicked.connect(lambda: self.add_acl_rule(
            acl_number_input, action_combo, protocol_combo, src_ip_input, dst_ip_input, dst_port_input, self.acl_table
        ))
        
        apply_button.clicked.connect(lambda: self.apply_acl_to_interface(
            interface_combo, start_port, end_port, direction_combo, acl_number_input
        ))
        ...
        # Console、SSH和Telnet登录配置卡片
        tab4.layout().addWidget(card_console)
    ...
    def create_card(self, title, icon, description=''):
        ...
        # divider.setStyleSheet('background-color: {t['border']};')
    ...
    def generate_config(self):
        ...
        if 'Console 串口登录配置' in checked_cards:
            if data.get('console_password'):
                ...
                config_commands.append(f"username {data.get('console_username')} password {data.get('console_user_password')}")
                config_commands.append("line console 0")
                config_commands.append("login local")
                config_commands.append("exit")
        
        if 'VLAN 基础配置' in checked_cards:
        ...
        # 处理远程登录配置（SSH和Telnet）
        ssh_configured = 'SSH登录配置' in checked_cards
        telnet_configured = 'Telnet登录配置' in checked_cards
        
        # 先处理Telnet配置（如果启用）
        if telnet_configured:
            # 收集Telnet认证信息
            telnet_auth_method = None
            telnet_username = None
            telnet_password = None
            
            if data.get('telnet_username') and data.get('telnet_user_password'):
                telnet_auth_method = 'local'
                telnet_username = data.get('telnet_username')
                telnet_password = data.get('telnet_user_password')
            elif data.get('telnet_pwd'):
                telnet_auth_method = 'password'
                telnet_password = data.get('telnet_pwd')
            
            if telnet_auth_method:
                # 配置vty线路
                config_commands.append("line vty 0 4")
                
                # 配置认证方式
                if telnet_auth_method == 'local':
                    config_commands.append("login local")
                else:  # password
                    config_commands.append("login")
                    config_commands.append(f"password {telnet_password}")
                
                # 配置允许的协议
                transport_input = []
                if ssh_configured:
                    transport_input.append("ssh")
                
                if transport_input:
                    config_commands.append(f"transport input {' '.join(transport_input)}")
                config_commands.append("exit")
            
            # 创建用户名（如果需要）
            if telnet_auth_method == 'local' and telnet_username and telnet_password:
                # 检查是否已经创建了用户名
                if not any(f"username {telnet_username}" in cmd for cmd in config_commands):
                    config_commands.append(f"username {telnet_username} password {telnet_password}")
        ...
        if ssh_configured:
            ...
            if data.get('ssh_username') and data.get('ssh_user_password'):
                ssh_auth_method = 'local'
                ssh_username = data.get('ssh_username')
                ssh_password = data.get('ssh_user_password')
            ...
            # 创建用户名（如果需要且Telnet未创建）
            if ssh_auth_method == 'local' and ssh_username and ssh_password:
                # 检查是否已经创建了用户名
                if not any(f"username {ssh_username}" in cmd for cmd in config_commands):
                    config_commands.append(f"username {ssh_username} password {ssh_password}")
            
            # 如果Telnet未配置，需要单独配置SSH的vty线路
            if not telnet_configured and ssh_auth_method:
                config_commands.append("line vty 0 4")
                
                # 配置认证方式
                if ssh_auth_method == 'local':
                    config_commands.append("login local")
                else:  # password
                    config_commands.append("login")
                    config_commands.append(f"password {ssh_password}")
            ...
            if hasattr(self, 'access_table') and self.access_table.rowCount() > 0:
                for row in range(self.access_table.rowCount()):
                    ...
                    if interface_item and start_item and end_item and vlan_item:
                        ...
                        # 构建端口范围字符串
                        port_range = f"{interface.replace(' ', '')}{start}-{end}"
                ...
                for row in range(self.agg_table.rowCount()):
                    ...
                    if agg_id_item and mode_item and lb_item and member_item:
                        ...
                        if match:
                            ...
                            # 配置成员端口
                            if mode == 'LACP':
                                config_commands.append(f"interface range {port_range.replace(' ', '')}")
        ...
        if '接口防环配置' in checked_cards:
            ...
            if data.get('loop_interface') and data.get('loop_start_port') and data.get('loop_end_port'):
                ...
                config_commands.append(f"int range {interface}{start_port}-{end_port}")
        ...
        if '生成树配置 (STP/RSTP/MSTP)' in checked_cards:
            ...
            if stp_mode == 'STP':
                config_commands.append("spanning-tree mode stp")
            elif stp_mode == 'RSTP':
                config_commands.append("spanning-tree mode rstp")
            elif stp_mode == 'MSTP':
                config_commands.append("spanning-tree mode mstp")
                config_commands.append("spanning-tree mst configuration")
                config_commands.append("exit")
            
            # 桥优先级
            if data.get('bridge_prio'):
                config_commands.append(f"spanning-tree priority {data.get('bridge_prio')}")
            
            # BDPU保护
            if data.get('bdpu_protection') and data.get('bdpu_protection') is True:
                config_commands.append("spanning-tree bpdu-protection")
            ...
            if hasattr(self, 'acl_table') and self.acl_table.rowCount() > 0:
                ...
                for row in range(self.acl_table.rowCount()):
                    ...
                    if acl_number_item and action_item and protocol_item and addr_item:
                        ...
                        # 构建规则命令
                        rule_command = f"access-list {acl_number} {action} {protocol}"
                        
                        # 处理源地址
                        if src_ip != 'any':
                            if src_ip.startswith('host '):
                                rule_command += f" {src_ip}"
                            else:
                                # 假设是网段格式，需要检查是否包含子网掩码
                                if ' ' in src_ip:
                                    rule_command += f" {src_ip}"
                                else:
                                    rule_command += f" {src_ip} 0.0.0.0"
                        else:
                            rule_command += " any"
                        ...
                        if dst_ip != 'any':
                            if dst_ip.startswith('host '):
                                rule_command += f" {dst_ip}"
                            else:
                                # 假设是网段格式，需要检查是否包含子网掩码
                                if ' ' in dst_ip:
                                    rule_command += f" {dst_ip}"
                                else:
                                    rule_command += f" {dst_ip} 0.0.0.0"
                        else:
                            rule_command += " any"
                        
                        # 处理目标端口
                        if dst_port:
                            rule_command += f" eq {dst_port}"
                        
                        # 添加到对应ACL编号的规则列表
                        if acl_number not in acl_rules:
                            acl_rules[acl_number] = []
                        acl_rules[acl_number].append(rule_command)
                ...
                for acl_number, rules in acl_rules.items():
                    ...
                    # 添加默认deny any规则
                    config_commands.append(f"access-list {acl_number} deny ip any any")
                
                # 处理ACL应用到接口
                if data.get('acl_interface') and data.get('acl_start_port') and data.get('acl_end_port') and data.get('acl_direction'):
                    interface = data.get('acl_interface').replace(' ', '')
                    start_port = data.get('acl_start_port')
                    end_port = data.get('acl_end_port')
                    direction = data.get('acl_direction')
                    acl_number = data.get('acl_number', '2000')
        ...
        # 生成完整单段配置
        self.preview_text.clear()
        config = ['enable', 'configure terminal'] + config_commands + ['end', 'write memory']
        config_str = '\n'.join(config)
        self.preview_text.setPlainText(config_str)
        return config_str
    ...
    def add_acl_rule(self, acl_number_input, action_combo, protocol_combo, src_ip_input, dst_ip_input, dst_port_input, acl_table):
        """添加ACL规则"""
        acl_number = acl_number_input.text().strip()
        action = action_combo.currentText()
        protocol = protocol_combo.currentText()
        src_ip = src_ip_input.text().strip()
        dst_ip = dst_ip_input.text().strip()
        dst_port = dst_port_input.text().strip()
        
        if not acl_number:
            from PyQt5.QtWidgets import QMessageBox
            QMessageBox.warning(self, "警告", "请填写ACL编号")
            return
        ...
        # 添加到表格
        row_position = acl_table.rowCount()
        acl_table.insertRow(row_position)
        
        # ACL编号
        acl_table.setItem(row_position, 0, QTableWidgetItem(acl_number))
        
        # 规则/动作
        acl_table.setItem(row_position, 1, QTableWidgetItem(rule_desc))
        
        # 协议
        acl_table.setItem(row_position, 2, QTableWidgetItem(protocol))
        
        # 源/目标地址
        acl_table.setItem(row_position, 3, QTableWidgetItem(addr_desc))
    ...
    def remove_acl_rule(self, acl_table, row):
        """删除ACL规则"""
    ...
    def apply_acl_to_interface(self, interface_combo, start_port, end_port, direction_combo, acl_number_input):
        """应用ACL到接口"""
        interface = interface_combo.currentText()
        start = start_port.text().strip()
        end = end_port.text().strip()
        direction = direction_combo.currentText()
        acl_number = acl_number_input.text().strip()
        
        if not interface or not start or not end or not acl_number:
            from PyQt5.QtWidgets import QMessageBox
            QMessageBox.warning(self, "警告", "请填写完整的接口范围和ACL编号")
            return
        
        # 验证端口号
        try:
            start_port_num = int(start)
            end_port_num = int(end)
            if start_port_num > end_port_num:
                QMessageBox.warning(self, "警告", "开始端口不能大于结束端口")
                return
        except ValueError:
            QMessageBox.warning(self, "警告", "端口号必须为数字")
            return
        
        # 显示成功消息
        from PyQt5.QtWidgets import QMessageBox
        QMessageBox.information(self, "成功", f"ACL {acl_number} 已成功应用到接口 {interface}{start}-{end} {direction} 方向")