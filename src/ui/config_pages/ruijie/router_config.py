...
class RuijieRouterConfig(BaseConfigPage):
    ...
    def init_panels(self):
        ...
        # 系统时间配置卡片
        card_time = self.create_card('系统时间配置', '🕒', '配置设备系统时间和NTP服务器')
        self.add_form_item(card_time, '系统时间', 'system_time')
        self.add_form_item(card_time, 'NTP服务器IP地址', 'ntp_server')
        ...
        tab6.content_layout.addStretch()
        ...
        tab7.content_layout.addStretch()
    ...
    def generate_config(self):
        ...
        if 'Console 串口登录配置' in checked_cards:
            if data.get('console_password'):
                ...
                config.append("enable")
                config.append("configure terminal")
                config.append(f"username {data.get('console_username')} privilege 15 secret {data.get('console_user_password')}")
                config.append("line console 0")
