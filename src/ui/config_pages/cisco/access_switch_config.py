    ...
    def generate_config(self):
        ...
        if 'DHCP Snooping功能' in checked_cards:
            if data.get('trust_port'):
                ...
                config_commands.append(f"interface {data.get('trust_port')}")
        ...
        if '接口防环配置' in checked_cards:
            if data.get('loop_port'):
                ...
                config_commands.append(f"interface range {data.get('loop_port')}")
        ...
        if 'ACL访问控制列表' in checked_cards:
            if data.get('allow_segment'):
                config_commands.append(f"access-list 10 permit {data.get('allow_segment')} 0.0.0.255")
