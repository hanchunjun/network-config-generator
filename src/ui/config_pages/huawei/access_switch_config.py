...
class HuaweiAccessSwitchConfig(BaseConfigPage):
    def __init__(self, parent):
        super().__init__(parent, 'huawei', 'access_switch')
        self.form_fields = {}
        self.cards = []
        self.init_panels()
    ...
    def init_panels(self):
        ...
        self.ssh_username_input.setFixedWidth(160)
        self.ssh_username_input.setPlaceholderText('请输入用户名')
        self.ssh_username_input.setStyleSheet(self._get_input_alt2_style())
        ssh_method2_layout.addWidget(self.ssh_username_input)
        ...
        self.ssh_user_password_input = QLineEdit()
        self.ssh_user_password_input.setEchoMode(QLineEdit.Password)
        self.ssh_user_password_input.setFixedWidth(160)
        self.ssh_user_password_input.setPlaceholderText('请输入密码')
        self.ssh_user_password_input.setStyleSheet(self._get_input_alt2_style())
        ssh_method2_layout.addWidget(self.ssh_user_password_input)
        self.form_fields['ssh_user_password'] = self.ssh_user_password_input
        ...
        self.telnet_username_input = QLineEdit()
        self.telnet_username_input.setFixedWidth(160)
        self.telnet_username_input.setPlaceholderText('请输入用户名')
        self.telnet_username_input.setStyleSheet(self._get_input_alt2_style())
        telnet_method2_layout.addWidget(self.telnet_username_input)
        ...
        self.telnet_user_password_input = QLineEdit()
        self.telnet_user_password_input.setEchoMode(QLineEdit.Password)
        self.telnet_user_password_input.setFixedWidth(160)
        self.telnet_user_password_input.setPlaceholderText('请输入密码')
        self.telnet_user_password_input.setStyleSheet(self._get_input_alt2_style())
        telnet_method2_layout.addWidget(self.telnet_user_password_input)
        self.form_fields['telnet_user_password'] = self.telnet_user_password_input
        ...
        card7 = self.create_card('链路聚合 (Eth-Trunk)', '🔗', '配置端口聚合，支持静态和动态LACP模式')
    ...
    def generate_config(self):
        ...
        if '设备基础信息' in checked_cards:
            ...
            if data.get('enable_password'):
                ...
                config_commands.append(f"local-user admin password irreversible-cipher {data.get('enable_password')}")
        ...
        if 'SSH登录配置' in checked_cards:
            if data.get('ssh_username') and data.get('ssh_user_password'):
                ...
                config_commands.append(f"quit")
            elif data.get('ssh_pwd'):
                config_commands.append(f"stelnet server enable")
                config_commands.append(f"user-interface vty 0 4")
                config_commands.append(f"authentication-mode password")
                config_commands.append(f"set authentication password cipher {data.get('ssh_pwd')}")
                config_commands.append(f"protocol inbound ssh")
                config_commands.append(f"quit")
        ...
        if '下联Access端口配置' in checked_cards:
            ...
            for row in range(self.access_table.rowCount()):
                ...
                if start_port and end_port and vlan:
                    # 华为专用批量接口命令格式
                    interface_type = interface  # 保持接口类型中的空格
                    config_commands.append(f"interface {interface_type}{start_port} to {interface_type.replace(' ', '')}{end_port}")
                ...
                for row in range(self.agg_table.rowCount()):
                    ...
                    if agg_id_item and mode_item and lb_item and member_item:
                        ...
                        if match:
                            ...
                            # 构建端口范围字符串
                            port_range = f"{interface_type}{interface_num}{start} to {interface_type}{interface_num}{end}"
                            ...
                            # 配置成员端口
                            config_commands.append(f"interface {port_range}")
        ...
        if 'DHCP Snooping功能' in checked_cards:
            ...
            if data.get('dhcp_snooping_interface') and data.get('dhcp_snooping_start_port') and data.get('dhcp_snooping_end_port'):
                interface = data.get('dhcp_snooping_interface')
                start_port = data.get('dhcp_snooping_start_port')
                end_port = data.get('dhcp_snooping_end_port')
                config_commands.append(f"interface {interface}{start_port} to {interface.replace(' ', '')}{end_port}")
                config_commands.append(f"dhcp snooping trusted")
                config_commands.append(f"quit")
        
        if '接口防环配置' in checked_cards:
            config_commands.append(f"loopback-detection enable")
            config_commands.append(f"loopback-detection recovery-time 120")
            if data.get('loop_interface') and data.get('loop_start_port') and data.get('loop_end_port'):
                interface = data.get('loop_interface')
                start_port = data.get('loop_start_port')
                end_port = data.get('loop_end_port')
                config_commands.append(f"interface {interface}{start_port} to {interface.replace(' ', '')}{end_port}")
                config_commands.append(f"stp bpdu-filter enable")
                config_commands.append(f"loopback-detection enable")
                config_commands.append(f"loopback-detection mode port-shutdown")
                config_commands.append(f"port-isolate enable group 1")
                config_commands.append(f"quit")
                config_commands.append(f"quit")
        ...
        if '生成树配置 (STP/RSTP/MSTP)' in checked_cards:
            ...
            if stp_mode == 'STP':
                config_commands.append(f"stp mode stp")
            elif stp_mode == 'RSTP':
                config_commands.append(f"stp mode rstp")
            elif stp_mode == 'MSTP':
            ...
            if hasattr(self, 'acl_table') and self.acl_table.rowCount() > 0:
                ...
                for row in range(self.acl_table.rowCount()):
                    ...
                    if acl_number_item and action_item and protocol_item and addr_item:
                        ...
                        rule_command = f"rule {action}"
                        if protocol != 'ip':
                            rule_command += f" protocol {protocol}"
                        if src_ip != 'any':
                            if src_ip.startswith('host '):
                                host_ip = src_ip.replace('host ', '')
                                rule_command += f" source {host_ip} 0"
                            else:
                                if ' ' in src_ip:
                                    ip_mask = src_ip.split()
                                    rule_command += f" source {ip_mask[0]} {ip_mask[1]}"
                                else:
                                    rule_command += f" source {src_ip} 0.0.0.0"
                        if dst_ip != 'any':
                            if dst_ip.startswith('host '):
                                host_ip = dst_ip.replace('host ', '')
                                rule_command += f" destination {host_ip} 0"
                            else:
                                if ' ' in dst_ip:
                                    ip_mask = dst_ip.split()
                                    rule_command += f" destination {ip_mask[0]} {ip_mask[1]}"
                                else:
                                    rule_command += f" destination {dst_ip} 0.0.0.0"
                        if dst_port:
                            rule_command += f" destination-port eq {dst_port}"
                        
                        acl_rules[acl_number].append(rule_command)
                
                for acl_number, rules in acl_rules.items():
                    config_commands.append(f"acl number {acl_number}")
                    for rule in rules:
                ...
                if data.get('acl_interface') and data.get('acl_start_port') and data.get('acl_end_port') and data.get('acl_direction'):
                    interface = data.get('acl_interface')
                    start_port = data.get('acl_start_port')
                    end_port = data.get('acl_end_port')
                    direction = data.get('acl_direction')
                    acl_number = data.get('acl_number', '3000')
                    
                    config_commands.append(f"interface range {interface}{start_port} to {interface.replace(' ', '')}{end_port}")
                    config_commands.append(f"traffic-filter {direction} acl {acl_number}")
                    config_commands.append(f"quit")
        ...
        if '端口镜像配置' in checked_cards:
            if data.get('mirror_src') and data.get('mirror_dst'):
                ...
                mirror_dir = data.get('mirror_dir', 'both')
                if mirror_dir == 'both':
                    config_commands.append(f"port-mirroring to observe-port 1 both")
                elif mirror_dir == 'rx':
                    config_commands.append(f"port-mirroring to observe-port 1 inbound")
                else:
                    config_commands.append(f"port-mirroring to observe-port 1 outbound")
                config_commands.append(f"quit")
        
        if 'AAA本地认证配置' in checked_cards:
            if data.get('aaa_user') and data.get('aaa_pwd'):
                config_commands.append(f"aaa")
                config_commands.append(f"local-user {data.get('aaa_user')} password irreversible-cipher {data.get('aaa_pwd')}")
                config_commands.append(f"local-user {data.get('aaa_user')} privilege level 15")
                config_commands.append(f"local-user {data.get('aaa_user')} service-type terminal ssh telnet")
                config_commands.append(f"quit")
